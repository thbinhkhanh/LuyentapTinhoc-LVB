  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        // ✅ Lấy tham số từ state parent (selectedLop, selectedBai)
        const lopHocParam = selectedLop;   // từ callback onSelectExam
        const tenBaiParam = selectedBai;   // từ callback onSelectExam
        ...
        // ✅ SỬ DỤNG COLLECTION VÀ DOC TỪ state
        const collectionName = `TRACNGHIEM${lopHocParam}`;
        const docId = tenBaiParam;

        let timeLimitMinutes = 45; // Mặc định nếu muốn, hoặc 0
        ...
        if (!docSnap.exists()) {
          const msg = `❌ Không tìm thấy đề ${docId}!`; // khai báo msg
          setSnackbar({
            open: true,
            message: msg,
            severity: "error",
          });
          setNotFoundMessage(msg); // dùng msg đã khai báo
          setLoading(false);
          return;
        }
        ...
        // 🔹 RESET trạng thái người dùng sau khi load câu hỏi
        setAnswers({});       // xóa tất cả các đáp án đã chọn
        setSubmitted(false);  // reset nút Nộp bài và màu đúng/sai
        setCurrentIndex(0);   // quay về câu đầu tiên
        setFillBlankStatus({}); // nếu dùng fillblank
        setProgress(100);
        setStarted(true);
